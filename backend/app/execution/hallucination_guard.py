...
class HallucinationGuard:
    ...
    CITATION_PATTERN = re.compile(
        r"\b(?:source|according to|cite[sd]?|reference|via|from)\b|https?://|\[[^\]]+\]\(http",
        re.IGNORECASE,
    )
    ...
    def assess(
        ...
        # Tool grounding
        assessment.tool_grounded = len(completed) > 0
        if research and not completed:
            risk += 0.35
            downgrade += 0.3
            penalty += 0.25
        ...
        if has_unsupported_phrase and not has_citation:
            risk += 0.25
            penalty += 0.2
        ...
        if (stat_matches or date_matches) and not has_citation and not completed:
            risk += 0.2
            downgrade += 0.15
        ...
        if entities and research and not completed:
            risk += 0.15
        ...
        if research and len(text.split()) > 80 and not completed:
            risk += 0.15
